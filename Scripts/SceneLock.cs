...
namespace AdvancedSceneManager.Plugin.Locking
{
    ...
    static class SceneLock
    {
        ...
        static bool OnLockButtonGUI(Rect position, Scene scene, ref float width)
        {
            ...
            _ = GUI.Toggle(position, isLocked, GetTooltip(scene.path), style);
            if (EditorGUI.EndChangeCheck())
                if (isLocked)
                    _ = LockUtility.PromptUnlock(scene.path);
                else
                    _ = LockUtility.PromptLock(scene.path);

            return true;

        }

        static bool OnWarningGUI(Rect position, Scene scene, ref float width)
        {

            var isLocked = LockUtility.IsLocked(scene.path);
            if (isLocked && scene.isDirty)
            {
                _ = GUI.Button(position, warning, GUIStyle.none);
                return true;
            }
